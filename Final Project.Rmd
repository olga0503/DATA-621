...
#split the variable 'Cabin' into 'Cabin floor' and "Cabin section"
data <- data %>% mutate(Cabin2=Cabin)  %>% separate(Cabin, c("Floor", "x"), sep = "[:digit:]", na.rm=T) %>% separate(Cabin2, c("Cabin2", "Room"), sep = "[:alpha:]", na.rm=T) %>% dplyr::select(-x, -Cabin2) 
...
All other variables have p-values that are greater than significance level of 5%. So, no transformation is needed.
...
Use stepwise approach to built the model.
